...
// clang can't figure out that these prototype should be defined, even if I
// define _SVID_SOURCE or _BSD_SOURCE, and _POSIX_C_SOURCE seems to already be
// defined.
...
void initX(char *geometry, char *winname) {
    int x, y, gravity;
    int gflags;
    ...
    gflags = XWMGeometry(dpy, screen_num, geometry, "300x200", border_width,
        size_hints, &x, &y, &width, &height, &gravity);
    if (!((gflags & WidthValue) && width)) {
        // If there's no width or they didn't specify one, make one up.
        width = display_width / 2;
    }
    if (!((gflags & HeightValue) && height)) {
        // If there's no height or they didn't specify one, make one up.
        height = display_height / 2;
    }
    if (!(gflags & XValue)) {
        x = 0;
#if 0
    } else if (gflags & XNegative) { // XWMGeometry fixes this up for us
    ...
    if (!(gflags & YValue)) {
        y = 0;
#if 0
    } else if (gflags & YNegative) { // XWMGeometry fixes this up for us
    ...
    if (!XGetIconSizes(dpy, RootWindow(dpy, screen_num), &size_list, &icount)) {
#ifdef DEBUG
        fprintf(stderr, "%s: WM didn't set icon sizes - using default.\n",
            progname);
#endif
    } else {
...
int init_colors() {
    ...
    static char *name[] = {
        ...
        "#ff0000", // red
        "#00ff00", // green
        "#5050ff", // blue // brighten up our blues a la Ken.P.  :-)
        "#00ffff", // aqua
        "#ff00ff", // magenta
        "#ffff00", // yellow
        "#ff7f7f", // pink
    ...
    if (i < StaticColor) { /* Color visual classes are 2 to 5 */
        /* No color visual available at default depth; some applications might
        call XMatchVisualInfo here to try for a GrayScale visual if they can use
        gray to advantage, before giving up and using black and white */
    ...
    /* The visual we found is not necessarily the default visual, and therefore
    it is not necessarily the one we used to create our window; however, we now
    know for sure that color is supported, so the following code will work (or
...
int procXevent(char *s, int n) {
    ...
    int nf, nfds, cn, in;
    // struct timeval *timer = NULL; // select blocks indefinitely
...
void xwin_draw_point(double x, double y) {
    if (displayon) {
        if (dd!= NULL) {
            XDrawPoint(dpy, *dd, gc, rint(x), height - rint(y));
            XDrawPoint(dpy, *dd, gc, rint(x) + 1, height - rint(y) + 1);
            XDrawPoint(dpy, *dd, gc, rint(x) + 1, height - rint(y));
            XDrawPoint(dpy, *dd, gc, rint(x), height - rint(y) + 1);
...
void xwin_set_pen_line(int pen, int line) {
    ...
    /* FIXME: Should avoid accessing out of bounds of colors[].  Also should
    cache different pen colors to avoid having to keep sending messages to
    server.  Currently this is enforced because ... ? */
    ...
    if (dd != NULL) {
        ...
        /* There are two switches here because even if you XSetDashes(), the
        drawn line will still be solid *unless* you also change the line style
        to LineOnOffDash or LineDoubleDash.  So we set line zero to be LineSolid
        and all the rest to LineOnOffDash and then the XSetDashes will take
        effect.  (45 minutes to debug!)

        Much appreciation to Ken Poulton for an example of this in autoplot(1)
        code. */
        ...
        switch (line) {
            ...
            case 7: /* long dotted */
                ...
                break;
            default:
                fprintf(stderr, "line type %d out of range, using solid\n",
                    line);
        }        
...
const char *xwin_ps_dashes(int line) {
    char *rv = NULL;
    switch (line % 7) {
        case 0: rv = "[] 0 sd"; break;
        case 1: rv = "[] 0 sd"; break;
        case 2: rv = "[7 5] 0 sd"; break;
        case 3: rv = "[7 2 1 2] 0 sd"; break;
        case 4: rv = "[7 2 3 2] 0 sd"; break;
        case 5: rv = "[9 5] 0 sd"; break;
        case 6: rv = "[4 4] 0 sd"; break;
    }
    return rv;
}

/* Any changes to this routine need to be harmonized with xwin_set_pen_line_fill
above. */
...
void xwin_annotate(PLOTDAT *pd, char *buf) {
    ...
    size = ((width + height) / 80.0);
    XClearArea(dpy, win, 0, height-(int)(size*1.5), width, 1.5*size, False);
    do_note(pd, buf, width / 2.0, 5.0, MIRROR_OFF, size, 1.0, 0.0, 0.0, 0, 1);
...
/* Given a shiftmask, return how many bits to shift it into position...  Look at
most max bits. */
...
int xwin_dump_graphics(char *cmd) {
    ...
    /* An X11 bug!  XGetImage(pixmap) does not set masks, so we get the masks
    from default_visual.  Thanks to
    <http://www.gnu-darwin.org/www001/ports-1.5a-CURRENT/x11-wm/windowmaker/work/WindowMaker-0.92.0/wrlib/xpixmap.c>
...
int xwin_dump_postscript(char *cmd) {
    ...
    ps_set_file(fp);
    // printf("%d %d\n", width, height);
    ps_preamble("title", "Pdplot", 8.5, 11.0, 0.0, 0.0,
