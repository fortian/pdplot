...
// initialized variables will get overridden by initplot()
void fontclip(int mode);

int numplots = 0;
PLOTDAT plots[MAXPLOTS];
static unsigned int nsegs = 0;
...
DATUM *datum_new(double x, double y) {
    DATUM *tmp;
...
#ifdef DEBUG
                    printf("plot %d: (%g, %g)->(%g, %g) %d %d\n",
                        i, x, y, x1, y1, buttonno, pressdown);
#endif
                    x2 = x1; y2 = y1;
                    return;
                } else if (buttonno == 3) {
...
// Just save the scaled points, so you don't have to bother about scaling again.
// However, this means that points read before the x or yscale will not get
// scaled.

void savepoint(double x, double y) {
    // scale the points as they are read
    // printf("linking %g %g\n", x/plots[0].xscale, y/plots[numplots].yscale);
    linkit(datum_new(x / plots[0].xscale, y / plots[numplots].yscale));
}
...
void dumppoints() {
    ...
    for (i = 0; i < MAXPLOTS; i++) {
        ...
        for (p = plots[i].data; p != NULL; p = p->next) {
            if (p->cmd == NULL) {
                printf("%d: %g %g\n", i, p->x, p->y);
            } else {
                printf("%d: %s\n", i, p->cmd);
    ...
    if (scalemode) {
        if (x) { // xaxis label
            ...
            do_note(pd, str, mid, tmp, MIRROR_OFF,
                scalesize * charsize * xfontsize * 0.6, 1.0, 0.0, 0.0, 0, 4);
        } else { // yaxis label
            tmp = pd->llx - pad - 2.0 * yfontsize;
            mid = (pd->lly+pd->ury) / 2.0;
            do_note(pd, str, tmp, mid, MIRROR_OFF,
                scalesize * charsize * yfontsize * 0.6, 1.0, 0.0, 90.0, 0, 4);
        }
    }
}
...
void render(void) {// this is where the image gets drawn
    int i;
    PLOTDAT *pd;
    DATUM *p;
    extern double ticklen, ticksize;
    double llx, lly, urx, ury, div, del, width, height, x, y, xold, yold,
           xx, yy, a, b, xmin, xmax, ymin, ymax, max, min, mid, uppx, uppy;
    char buf[128];
    ...
    // We allow each graph to be 5 units tall, and spacing between graphs is one
    // unit.
    ...
    // Now we do a dry run through the dataset using the bounds calculation code
    // in clip to set plot boundaries w.r.t. xset, yset commands.
    ...
    for (i = 0; i <= numplots; i++) {
        ...
        // FIXME: only need to do this next loop through the data if either xset
        // or yset is used.  Otherwise, just use the data bounds...
    ...
    for (i = 0; i <= numplots; i++) {
        ...
        pd->llx = llx;
        pd->lly = lly + 6.0 * del * i;
        pd->urx = urx;
        pd->ury = lly + (5.0 + 6.0 * i) * del;

#ifdef DEBUG
        printf("numplots = %d: %g %g %g %g\n",
        ...
        for (p = pd->data; p != NULL; p = p->next) {
            ...
            if (p->cmd == NULL) {
                ...
                if (sscanf(p->cmd, "%*s %lg%% %lg%% %[^#]", &x, &y, buf) == 3) {
                    ...
                    do_note(pd, buf, xx, yy, MIRROR_OFF,
                        0.6 * fontsize * charsize * labelsize, 1.0, 0.0,
                        labelangle, 0, 3);
                } else if (sscanf(p->cmd, "%*s %lg %lg %[^#]", &x, &y, buf) == 3) {
                    xx = (pd->urx - pd->llx)*(logscale(x, 0) - xmin) /
                        (xmax - xmin)+pd->llx;
                    yy = (pd->ury - pd->lly)*(logscale(y, 1) - pd->ymin) /
                        (pd->ymax - pd->ymin)+pd->lly;
        ...
        printf("calling ll: %g %g %g %g\n", xmin, pd->ymin, xmax, pd->ymax);
#endif
        loose_label(pd, &pd->ymin, &pd->ymax, 20 / (numplots + 2), 0, 1,
            pd->ylogmode, 0);
        loose_label(pd, &xmin, &xmax, 8, 1, !i, plots[0].xlogmode, 0);		
#ifdef DEBUG
        printf("%g %g %g %g\n", xmin, pd->ymin, xmax, pd->ymax);
#endif
    }
...
#ifdef DEBUG
    printf("in mat_slant with theta = %g\n", theta);
    mat_print(xp);
#endif
...
void do_note(PLOTDAT *pd, char *string, double x, double y, int mirror,
    double size, double aspect, double slant, double rotation, int id, int jf) {
    XFORM *xp;
    ...
    /* NOTE: To work properly, these transformations have to occur in the proper
       order; for example, rotation must come after slant transformation. */
