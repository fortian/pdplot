...
// Get a line of no more than n chars.  Return 0 on success, EOF on end of file.
// Chop of any trailing newline.

#define getz(s, n) procXevent((s), (n))

// return a pointer to the first non-blank character in s
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "g:n:t:")) != -1) {
        switch (opt) {
            //case 'r':
            ...
            default: /* '?' */
                fprintf(stderr,
                    "usage: %s [-g geometry] [-n nwin] [-t win-title] <file/script>\n",
                    argv[0]);
                return 1;
        }
    }

    initX(geometry, wintitle);
    initsym();
    initplot();
